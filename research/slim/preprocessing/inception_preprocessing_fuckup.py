...
def preprocess_for_eval(image,
  ...
  with tf.name_scope(scope, 'eval_image', [image, height, width]):
    ...
    #def f1():
    #    image = tf.image.resize_image_with_crop_or_pad(image, min_size, min_size)
    ...
    #image = tf.cond(tf.math.greater(tf.cast(tf.math.multiply(min_size * central_fraction), tf.float32), tf.cast(height, tf.float32)), f1, f2)

#    if tf.math.greater((tf.cast(h * crop_size, tf.int32)), height) and tf.math.greater(tf.cast((w * crop_size, tf.int32)), width):
